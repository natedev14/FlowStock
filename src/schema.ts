...
export function validateRows(rows: CsvRow[]): { ok: true } | { ok: false; message: string } {
  ...
  if (emptyCodeRows.length > 0) {
    return {
      ok: false,
      message: `Há linhas sem Código. Revise as linhas: ${emptyCodeRows.slice(0, 8).join(', ')}.`,
    };
  }
  ...
  if (duplicateCodes.size > 0) {
    return {
      ok: false,
      message: `Há códigos duplicados: ${Array.from(duplicateCodes).slice(0, 8).join(', ')}. Corrija o arquivo antes da contagem.`,
    };
  }
  ...
  if (parents.length === 0) {
    return {
      ok: false,
      message: 'Nenhum produto pai foi encontrado. O arquivo deve ter uma linha com Código Pai vazio ou 0.',
    };
  }
  ...
  if (parents.length > 1) {
    return {
      ok: false,
      message: `Este arquivo contém ${parents.length} produtos pai. Para evitar erros no estoque, o FlowStock trabalha com 1 produto por CSV. Exporte ou carregue apenas um produto por vez.`,
    };
  }
  ...
  if (children.length === 0) {
    return {
      ok: false,
      message: 'Este produto não possui variações para contagem.',
    };
  }
